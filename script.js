document.addEventListener('DOMContentLoaded', () => {
    ...
    function displayStatsAndConfigs(textContent, countryFileName) {
        configContainer.innerHTML = ''; // پاک کردن محتوای قبلی
        ...
        const stats = {
            VLESS: 0, VMESS: 0, TROJAN: 0, SS: 0, HY2: 0, OTHER: 0
        };
    ...
    function showCountryList() {
        ...
        document.querySelectorAll('.country-item').forEach(item => item.style.display = 'block');
    ...
    async function loadCountries() {
        try {
            ...
            if (!response.ok) throw new Error('Failed to fetch country list from GitHub API');
            ...
            const countryPromises = files
                .filter(file => file.type === 'file' && file.name.endsWith('.txt'))
                .map(async file => {
                    try {
                        const res = await fetch(`loc/${file.name}?v=${new Date().getTime()}`);
                        if (!res.ok) return { file, count: 0 };
            ...
            const countriesWithCounts = await Promise.all(countryPromises);

            countriesWithCounts.sort((a, b) => a.file.name.localeCompare(b.file.name));

            countriesWithCounts.forEach(({ file, count }) => {
                if (count === 0) return;
                ...
                countryItem.innerHTML = `
                    <div class="country-info">
                        ...
                        <small>${countryCode} / ${countryData.configCount} کانفیگ موجود</small>
                    </div>
                `;
            ...
            loader.textContent = 'خطا در بارگیری لیست کشورها. لطفاً صفحه را رفرش کنید.';
